...
function renderGap(payload) {
  ...
  box.innerHTML = table([
    ...
    { key: "prob", label: "模型概率", format: (v) => (v === null || v === undefined || Number.isNaN(v) ? "--" : (v * 100).toFixed(1) + "%") },
    { key: "enhanced_prob", label: "综合概率", format: (v) => (v === null || v === undefined || Number.isNaN(v) ? "--" : (v * 100).toFixed(1) + "%") },
