...
独立页签「尾盘高开」：T日尾盘买入候选，次日开盘 ≥ +3% 获利了结。

- 信号过滤：前日涨停/连板、主力净流入、排除解禁/减持，只保留 Top3
- 自动验证：T+1 开盘记录是否高开 ≥ +3%，写入成交记录和命中率
- 页面显示：今日候选、当前持仓、历史成绩（单笔/交易日/近30天命中率）

当前模型回测基线：Top3 单日命中率 58.1%；目标是通过竞价确认和更严格过滤
把“Top3 至少一只高开 +3%”的命中率提升到 80% 区间。
...
结论：按“每批 Top3 篮子”口径，80%+ 目标可以达成，适合作为尾盘买入策略主口径。

尾盘模型已固化为 `tail_reach_model.json` 并接入「尾盘买入」页签：
全市场扫描 + 专属模型打分，T+1 开盘或盘中最高达 +3% 即算命中；
回测 Top3 命中 89.2%、Top1 63.5%。

「次日高开」页签已整合尾盘逻辑：沿用主板/ST/价格/行业筛选，
再用尾盘专属 GBDT 预测“盘中达 +3%”概率，只展示 Top3。
