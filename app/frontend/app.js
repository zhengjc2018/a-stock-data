...
function renderGap(payload) {
  ...
  box.innerHTML = table([
    ...
    { key: "turnover_pct", label: "换手", format: (v) => (v === null || v === undefined ? "--" : fmt(v, 1) + "%"), hideSm: true },
...
async function loadStrategyHealth() {
  ...
  try {
    ...
    const cards = [
      ...
      ["做T平均净收益", ts.verified && ts.avg_net_ret !== null && ts.avg_net_ret !== undefined ? (ts.avg_net_ret * 100).toFixed(2) + "%" : "--"],
